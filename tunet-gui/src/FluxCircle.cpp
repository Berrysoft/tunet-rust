...
static void draw_arc(QPainter& painter, const QPen& pen, const QPointF& center, double radius, double startAngle, double arcLength)
{
    ...
    path.lineTo(center.x() + small_radius * std::cos((startAngle + arcLength) / 180.0 * pi), center.y() + small_radius * std::sin((startAngle + arcLength) / 180.0 * pi));
    path.arcTo(small_rect, startAngle + arcLength, -arcLength);
    path.closeSubpath();
